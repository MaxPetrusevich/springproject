function toggleSubscriptionStatus(id) {
    fetch(`/organiser/subscriptions/${id}/toggle`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            const statusBadge = document.querySelector(`#subscription-${id} .status-badge`);
            const toggleButton = document.querySelector(`#subscription-${id} .toggle-status-btn`);
